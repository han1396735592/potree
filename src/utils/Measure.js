...
export class Measure extends THREE.Object3D {
    constructor(type='Measure') {
        super();

        this.constructor.counter = (this.constructor.counter === undefined) ? 0 : this.constructor.counter + 1;
        this.name = type + '_' + this.constructor.counter
        this.points = [];
    ...
    addMarker(point) {
        ...
        { // coordinate labels
            ...
            let msg = point.position.toArray().map(p => Utils.addCommas(p.toFixed(2))).join(' / ')
    ...
    update() {
        ...
        for (let i = 0; i <= lastIndex; i++) {
            ...
            { // coordinate labels
                ...
                let msg = point.position.toArray().map(p => Utils.addCommas(p.toFixed(2))).join(' / ')
                coordinateLabel.setText('(' + (index + 1) + ')' + msg)
                // coordinateLabel.visible = this.showCoordinates;
            }
